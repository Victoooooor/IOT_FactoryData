...
#define OLED_CLK_PIN	HAL_GPIO_PIN_P4_6
#define OLED_SDA_PIN	HAL_GPIO_PIN_P4_0
#define OLED_RES_PIN	HAL_GPIO_PIN_P4_1
#define OLED_DC_PIN		HAL_GPIO_PIN_P2_7
#define OLED_CS_PIN		HAL_GPIO_PIN_P4_7
#define CTRL_BD_PIN		HAL_GPIO_PIN_P0_5
...
static void OLED_GPIO_Set(unsigned char port,unsigned char leve)
{
	switch(port){
		...
		break;
		case CS_PORT:
			oled_gpio.port = OLED_CS_PIN;
		break;
		case CTRL_PORT:
			oled_gpio.port = CTRL_BD_PIN;
		default:
			LOGI("APP","Unhandled PIN num Error\n");
		break;
	}
	if(leve == 1){
...
void OLED_CS( uint8_t x){
	if(x==1){
		OLED_GPIO_Set(CS_PORT,1);
	}
	else if(!x){	
		OLED_GPIO_Set(CS_PORT,0);
	}
	else{
		LOGI("APP","OLED set input value err\n");
	}
}
...
{	//OLED_CS(1);
	...
	/* reset */
	OLED_RES(0);
	OLED_CS(1);//by https://www.velleman.eu/downloads/29/infosheets/sh1106_datasheet.pdf pg.11 : serial in initialized @ CS^ high
	_msdelay(1000);
	OLED_CS(0);
	OLED_RES(1);
...
/**
 * @brief oled gpio init 初始化OLED屏幕
 * 
 * @return 0 : on Success, <0 : Error
 */
int oled_gpio_init(void)
{
	name_t* lock_name="OLED_LOCK";
    krhino_mutex_create(&lock,lock_name);

	int ret = 0;

	oled_gpio.port = OLED_CLK_PIN;
    oled_gpio.config = OUTPUT_PUSH_PULL;
    ret += hal_gpio_init(&oled_gpio);
	oled_gpio.port = OLED_SDA_PIN;
    oled_gpio.config = OUTPUT_PUSH_PULL;
    ret += hal_gpio_init(&oled_gpio);
	oled_gpio.port = OLED_RES_PIN;
...
void _disp(byte line, void *context, byte mode){
	...
	case 0:
		for(byte i = 5; i < 8; i++){
			for(byte j = 0; j < OLED_PAGE_SIZES; j++)
			OLED_GRAM[j][i]=0x0;
		}
		OLED_Show_String(start,7,text,12,mode);
		break;
	...
	case 1:
		...
		OLED_Show_String(start,26,text,12,mode);
		break;
	...
	case 2:
		...
		OLED_Show_String(start,45,text,12,mode);
		break;
...
/**
 * @brief Create Scrolling Affect on OLED Screen, not used.
 * 
 */
void scroll(){
	int x=0x40;
	...
	while(1){
		aos_msleep(100);
		if(x>=0x40 && x<0x80)
			OLED_WR_Cmd(x);//changing start display line -> scorlling effect
		x++;
		x%=0x80;
		if(!x){
			x+=0x40;
		...
		count%=100;
		if(!count){
			LOGI("APP","DEBUG: in OLED init\n");
		}
	}
}

void ctrl_set(unsigned short lvl){
	OLED_GPIO_Set(CTRL_PORT,lvl);
}